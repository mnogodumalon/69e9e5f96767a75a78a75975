...
interface VerbrauchsstatistikDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Verbrauchsstatistik['fields']) => Promise<void>;
  defaultValues?: Verbrauchsstatistik['fields'];
  produktkatalogList: Produktkatalog[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function VerbrauchsstatistikDialog({ open, onClose, onSubmit, defaultValues, produktkatalogList, enablePhotoScan = true, enablePhotoLocation = true }: VerbrauchsstatistikDialogProps) {
  const [fields, setFields] = useState<Partial<Verbrauchsstatistik['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'verbrauchsstatistik');
      await onSubmit(clean as Verbrauchsstatistik['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="statistik_produkt_ref" entity="Produktkatalog">\n${JSON.stringify(produktkatalogList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "zeitraum_von": string | null, // YYYY-MM-DD\n  "zeitraum_bis": string | null, // YYYY-MM-DD\n  "statistik_produkt_ref": string | null, // Display name from Produktkatalog (see <available-records>)\n  "gesamtverbrauch_rollen": number | null, // Gesamtverbrauch im Zeitraum (Rollen)\n  "durchschnitt_pro_person_tag": number | null, // Durchschnittlicher Verbrauch pro Person pro Tag (Rollen)\n  "statistik_personen": number | null, // Anzahl Personen\n  "gesamtkosten": number | null, // Gesamtkosten im Zeitraum (€)\n  "kosten_pro_rolle": number | null, // Kosten pro Rolle (€)\n  "kosten_pro_person_monat": number | null, // Kosten pro Person pro Monat (€)\n  "effizienzeinschaetzung": LookupValue | null, // Effizienzeinschätzung (select one key: "sehr_effizient" | "effizient" | "durchschnittlich" | "verbesserungswuerdig" | "verschwenderisch") mapping: sehr_effizient=Sehr effizient, effizient=Effizient, durchschnittlich=Durchschnittlich, verbesserungswuerdig=Verbesserungswürdig, verschwenderisch=Verschwenderisch\n  "optimierungsempfehlungen": string | null, // Optimierungsempfehlungen\n  "notizen_statistik": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["statistik_produkt_ref"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const statistik_produkt_refName = raw['statistik_produkt_ref'] as string | null;
        if (statistik_produkt_refName) {
          const statistik_produkt_refMatch = produktkatalogList.find(r => matchName(statistik_produkt_refName!, [String(r.fields.marke ?? '')]));
          if (statistik_produkt_refMatch) merged['statistik_produkt_ref'] = createRecordUrl(APP_IDS.PRODUKTKATALOG, statistik_produkt_refMatch.record_id);
        }
        return merged as Partial<Verbrauchsstatistik['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Verbrauchsstatistik bearbeiten' : 'Verbrauchsstatistik hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="statistik_produkt_ref">Ausgewertetes Produkt</Label>
            <Select
              value={extractRecordId(fields.statistik_produkt_ref) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, statistik_produkt_ref: v === 'none' ? undefined : createRecordUrl(APP_IDS.PRODUKTKATALOG, v) }))}
            >
              <SelectTrigger id="statistik_produkt_ref"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
